	  		...
	  		function atualizaAcaoClickCategoria(){
	  			...
	  			$(".menuCarrosselItem").click(function () {
	  				...
	  				var labelCategoria = $(this).find(".identificadorTituloCategoria").html();
	  				...
	  				$("#categoriaTituloSelecionado").val(labelCategoria);
	  				...
	  				$("#btoExcluirCategoria").css("display","block");
	  				$("#gerenciarSubMenu").css("display","block");				
	  				
	  				$("#categoria_excluir").html(labelCategoria);
	  			});	
		  ...
		  $("#ordenacao").click(function(){
		  	...
		  	if($("#ordenacao").prop("checked")){
		  		...
		  		if(listaChanged){
			  		if(confirm("Voc&eacute; alterou a ordem das categorias;\nDeseja gravar e manter suas altera&ccedil;&otilde;es?")){
			  			listaChanged=false;
		  ...
		  $("#novaCategoria").click(function(){
			...
			var dispositivo = $("#comboDispositivo").val();
			var segmento = $("#comboSegmento").val();
			
			
			var local = $("#novaCategoriaHref").prop("href") +"/"+ambiente+"/"+segmento+"/"+dispositivo;
			$("#listaCategoriaForm").prop("action",local);
			$("#listaCategoriaForm").submit();
		...
		function listarMenuCarrossel(){
			...
			if(ambiente!="" && ambiente!="0" && segmento!="" && segmento!="0" && dispositivo!="" && dispositivo!="0"){
				...
				$("#divListaCategorias").html("<div style='width:100%;text-align:center'>Selecione uma categoria e um dispositivo.</div>");
			}
		}
